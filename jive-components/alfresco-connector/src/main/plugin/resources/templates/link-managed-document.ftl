...
<head>
    ...
    <#else>
        <title><@s.text name="doc.managed.link.title" /></title>
    </#if>
    ...
    <div class="jive-create-doc jive-create-large jive-content doc-page clearfix">
            ...
            <header>
              <h2>
                  ...
                  <#else>
                  <@s.text name="doc.managed.link.title" />
                  </#if>
        ...
        <#assign formAction><#if edit><@s.url action='doc-upload-edit' /><#else><@s.url action='doc-upload' /></#if></#assign>
        ...
        <form class="j-form" action="doc-managed-link.jspa" method="post" enctype="multipart/form-data" name="postform" id="postform" onsubmit="return validateUploadPost(true);">
        <input type="hidden" name="containerType" value="${container.objectType?c}" />
        <input type="hidden" name="container" value="${container.ID?c}" />
        <input type="hidden" name="remoteDocument" value="${remoteDocument}" />
        <input type="hidden" name="uploadFileFileName" value="${uploadFileFileName}" />
        <input type="hidden" name="uploadFileContentType" value="${uploadFileContentType}" />
        <input type="hidden" name="uploadFileSize" value="${uploadFileSize?c}" />
